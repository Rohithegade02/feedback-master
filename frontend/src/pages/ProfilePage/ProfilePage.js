...
const ProfilePage = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/user/get/${decodeSessionStorage()._id}`)
        .then(res => {
            setUser(res.data)
    ...
    const onSave = async () => {
        if(newEmail !== user.contact.email || newPhone !== user.contact.phone_number) {
            await axios.post(`${process.env.REACT_APP_API_URL}/user/update/contact`, {
        ...
        if(newPassword){
            await axios.post(`${process.env.REACT_APP_API_URL}/user/update/password`, {
                    username: user.username, 
                    password, 
                    newPassword
                }
