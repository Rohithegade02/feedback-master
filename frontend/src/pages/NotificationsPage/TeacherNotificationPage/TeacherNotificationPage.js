...
const TeacherNotificationPage = () => {
    ...
    useEffect(() => {
        if(params.id) {
            axios.get(`${process.env.REACT_APP_API_URL}/notification/get/${params.id}`, { headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` } })
    ...
    useEffect(() => {
        axios.post(
            `${process.env.REACT_APP_API_URL}/notification/get/all/${decodeSessionStorage().school_id}`,
            {
                offset: 7*(page-1),
                limit: 7
            },
