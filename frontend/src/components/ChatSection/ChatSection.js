...
const ChatSection = ({update, setUpdate}) => {
    ...
    useEffect(() => {
        if(params.id) {
            axios.get(`${process.env.REACT_APP_API_URL}/chat/get/${params.id}`, { headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` } })
            .then(res => {
    ...
    const onSendMessage = async () => {
        axios.post(
            `${process.env.REACT_APP_API_URL}/chat/register`, 
            {
                text: message,
    ...
    const onMarkAsResolved = async() => {
        axios.get(`${process.env.REACT_APP_API_URL}/complaint/${selectedComplaint._id}/resolved`, { headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` } })
        .then(res => {
